...
export function FooterParticles() {
  ...
  useEffect(() => {
    ...
    const pts: P[] = Array.from({ length: N }, () => { const p = { x: 0, y: 0, z: 1, ph: 0, sp: 1 }; spawn(p, true); return p; });
    ...
    const draw = (t: number, dt: number) => {
      ...
      for (const p of pts) {
        p.z -= SPEED * dt;
        if (p.z <= Z_NEAR) { spawn(p, false); continue; }
        const sx = cx + (p.x / p.z) * scale;
        const sy = cy + (p.y / p.z) * scale;
        if (sx < -10 || sx > w + 10 || sy < -10 || sy > h + 10) { spawn(p, false); continue; }
        ...
        const a = Math.min(1, (0.2 + 0.8 * near) * fadeIn * twinkle);
